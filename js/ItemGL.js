...
(function($){
  ...
  var methods = {
		...
		init: function(settings){
			...
			options.numObj = parseInt(options.objGeometry.length);
      ...
      if(isiPad || isiPhone){
		...
		onDocumentMouseDown: function(event){
			...
			if(options.isMobile){
				if(event.changedTouches[0].pageX<51){
					realPointer.x = null;
				} else if(event.changedTouches[0].pageX>50 && event.changedTouches[0].pageX<window.innerWidth-300){
					realPointer.x = event.changedTouches[0].pageX-50;
				} else{
					realPointer.x = null;
				}

				if(event.changedTouches[0].pageY<51){
					realPointer.y = null;
				} else if(event.changedTouches[0].pageY>50 && event.changedTouches[0].pageY<window.innerHeight-50){
					realPointer.y = event.changedTouches[0].pageY-50;
				} else {
					realPointer.y = null;
				}
			} else {
				...
				if(event.clientX<51){
					realPointer.x = null;
				} else if(event.clientX>50 && event.clientX<window.innerWidth-300){
					realPointer.x = event.clientX-50;
				} else{
					realPointer.x = null;
				}
				...
				if(event.clientY<51){
					realPointer.y = null;
				} else if(event.clientY>50 && event.clientY<window.innerHeight-50){
					realPointer.y = event.clientY-50;
				} else{
					realPointer.y = null;
				}
			}
